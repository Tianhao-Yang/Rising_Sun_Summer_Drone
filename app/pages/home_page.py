...
from core.public_log_publisher import publish_flight_folder
...
class HomePage(QWidget):
    ...
    def __init__(self):
        ...
        self.publish_button = QPushButton("Publish to Website")
        self.publish_button.setToolTip(
            "Publish the selected flight to public_logs and Cloudflare R2"
        )
        self.publish_button.clicked.connect(self._publish_selected_flight)
        self.publish_finished.connect(self._on_publish_finished)
        ...
        logs_header.addWidget(self.publish_button)
        ...
        guiding_principle_panel = QFrame()
        guiding_principle_panel.setObjectName("guidingPrinciplePanel")
        guiding_principle_panel.setMaximumWidth(900)
        guiding_principle_panel.setFixedHeight(255)
        guiding_principle_panel.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed
        )
        guiding_principle_panel.setStyleSheet(
            "QFrame#guidingPrinciplePanel {"
            "  background: #980000; border: 1px solid #720000;"
            "  border-radius: 6px;"
            "}"
        )

        principle_layout = QHBoxLayout(guiding_principle_panel)
        principle_layout.setContentsMargins(62, 22, 62, 22)
        principle_layout.setSpacing(72)

        # Displayed left-to-right here, but read in traditional order from right.
        vertical_columns = (
            ("周\n恩\n来", 29),
            ("万\n无\n一\n失", 37),
            ("稳\n妥\n可\n靠", 37),
        ...
        principle_layout.addStretch(1)
        for column_text, font_size in vertical_columns:
            column = QLabel(column_text)
            column.setAlignment(Qt.AlignCenter)
            column.setStyleSheet(
                "QLabel {"
                "  color: #f0d39a; background: transparent; border: none;"
                "  font-family: 'STXingkai', '华文行楷', 'KaiTi', '楷体';"
                f"  font-size: {font_size}px; font-weight: 600;"
                "}"
            )
            principle_layout.addWidget(column)
        principle_layout.addStretch(1)
        ...
        body_layout.addWidget(
            guiding_principle_panel, alignment=Qt.AlignHCenter
        )
...
<body>
  ...
  <div class="hint">Drag to rotate · Scroll to zoom</div>
    ...
    def _selected_flight_folder(self):
        """Return the folder explicitly selected by the user."""
        if self.current_log_folder is not None:
            folder = Path(self.current_log_folder)
    ...
    def _publish_selected_flight(self):
        folder = self._selected_flight_folder()
        if folder is None:
            QMessageBox.information(
                self,
                "Select a Flight",
                "Select a flight folder first, then click Publish to Website.",
            )
            return
        ...
        answer = QMessageBox.question(
            self,
            "Publish Flight",
            f"Publish {folder.name} to the public website?\n\n"
            "Only CSV, session.json, and browser-compatible *_web.mp4 files "
            "will be published. Original MP4 files remain private.",
        )
        ...
        def work():
            try:
                message = publish_flight_folder(folder)
                self.publish_finished.emit(True, message)
            except Exception as error:
                self.publish_finished.emit(False, str(error))

        threading.Thread(target=work, daemon=True).start()

    def _on_publish_finished(self, succeeded, message):
        self.publish_button.setEnabled(True)
        self.publish_button.setText("Publish to Website")
        if succeeded:
            QMessageBox.information(self, "Flight Published", message)
        else:
